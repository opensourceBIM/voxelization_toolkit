...
TEST(Voxelizer, ChunkPrimitives) {
	...
	EXPECT_TRUE(storage->get_chunk(make_vec<size_t>(0U, 0U, 0U))->is_explicit());
	std::array< vec_n<3, size_t>, 2 > out;
	storage->chunk_extents(make_vec<size_t>( 1U,1U,0U), out);
	std::cerr << out[0].format() << " - " << out[1].format() << std::endl;
	EXPECT_FALSE(storage->get_chunk(make_vec<size_t>(1U, 1U, 0U))->is_explicit());
	...
	EXPECT_TRUE(storage->get_chunk(make_vec<size_t>(0U, 0U, 0U))->is_explicit());
	EXPECT_FALSE(storage->get_chunk(make_vec<size_t>(1U, 1U, 0U))->is_explicit());
	EXPECT_FALSE(storage->get_chunk(make_vec<size_t>(2U, 1U, 0U))->is_explicit());
	...
	storage->get_chunk(make_vec<size_t>(1U, 1U, 0U))->write(file_part_primitives, ss);
	EXPECT_EQ(ss.str(), "Z=0,Z=5,Z=10");

	ss.str("");
	EXPECT_FALSE(storage->get_chunk(make_vec<size_t>(2U, 1U, 0U))->is_explicit());
	storage->get_chunk(make_vec<size_t>(2U, 1U, 0U))->write(file_part_primitives, ss);
