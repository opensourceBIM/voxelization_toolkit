...
TEST(DownSampling, Constant) {
	...
	storage->create_constant({ 0U,0U,0U }, 1);
...
TEST(UpSampling, Plane) {
	auto storage = new chunked_voxel_storage<bit_t>(0., 0., 0., 0.1, 100, 100, 100, 32);
	// Z=20
	storage->create_plane_primitive({ 0U,0U,0U }, 2, 20);
	auto result = (chunked_voxel_storage<bit_t>*) resampler(+2)(storage);
	ASSERT_TRUE(result->get_chunk({ 0U, 0U, 0U }) == nullptr);
	ASSERT_FALSE(result->get_chunk({ 0U, 0U, 1U })->is_explicit());
	// Z = (20 * 2) % 32 = 8
	ASSERT_TRUE(result->get_chunk({ 0U, 0U, 1U })->Get({ 0U, 0U, 8U }));
}
...
TEST(UpSampling, Continuous) {
	...
	auto result = (chunked_voxel_storage<bit_t>*) resampler(+2)(storage);
	ASSERT_FALSE(result->Get({ 1U, 1U, 1U }));
	ASSERT_TRUE(result->Get({ 2U, 2U, 2U }));
	ASSERT_TRUE(result->Get({ 3U, 3U, 3U }));
	ASSERT_FALSE(result->Get({ 4U, 4U, 4U }));
}
